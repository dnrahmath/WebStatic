...
func main() {
	...
	r.Use(func(c *gin.Context) {
		allowedOrigins := []string{
			...
			"http://127.0.0.1", "http://192.168.1.10", "http://localhost", "http://localhost:8394",
		}
	...
	r.GET("/index-glb", func(c *gin.Context) {
		c.File("./index-glb.html")
	})

	r.GET("/pick", func(c *gin.Context) {
		c.File("./Pick-Material-Design-Color-Chart.html")
	})
	...
	fmt.Println("------------------------------------------------------")
	fmt.Println("http://localhost:8394/")
	fmt.Println("------------------------------------------------------")
	fmt.Printf("http://%s:%s/\n", url, port)
